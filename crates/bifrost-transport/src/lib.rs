...
/// discovery (raw QUIC) pairs with a real resolver ([`StaticDiscovery`], mDNS, later pkarr). Sources
/// compose: [`Layered`] tries two together and unions their hints, so an explicit and a learned
/// source can both feed one dial.
#[allow(async_fn_in_trait)]
pub trait Discovery {
